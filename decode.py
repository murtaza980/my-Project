...
farmers_dict = {
  "FARM": str(hashlib.sha256("FARM".encode()).hexdigest()),
  "مصنع صحاري كربلاء": str(hashlib.sha256("مصنع صحاري كربلاء".encode()).hexdigest()),
  "شركة كالة": str(hashlib.sha256("شركة كالة".encode()).hexdigest()),
  "جاي محمود": str(hashlib.sha256("جاي محمود".encode()).hexdigest()),
  "حلويات الشكرجي": str(hashlib.sha256("حلويات الشكرجي".encode()).hexdigest())
}

#qr = qrtools.QR()
#qr.decode("qrcode_apple.png")
for i in range(1, 11):
    qr = decode(Image.open(f'qrcode_{i}.png'))
